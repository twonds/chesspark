...
unsigned char *SHA1_GetBinDigestForFile(const char *filename)
{
	SHA1_CTX context;
	unsigned char *digest, buffer[16384];
    ...
    while (!feof(fp))
	{
        int i = (int)fread(buffer, 1, 16384, fp);
        SHA1Update(&context, buffer, i);
    }
...
char *SHA1_GetHexDigestForFile(const char *filename)
{
	...
	if (!(digest = SHA1_GetBinDigestForFile(filename)))
	{
		return NULL;
	}
